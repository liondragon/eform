...
This folder contains stable implementation-facing contracts that are too detailed for `README.md` and too broad for a single owner entry.

Use these files only for contracts that cross public surfaces, template authorship, runtime storage, or machine-readable outputs. Keep local owner routing in `docs/Owner_Index.md`, subsystem routing in `docs/Architecture_Router.md`, and operator guidance in root `README.md`.

...
- [x] EFORMS-FIX-006 Clear-win helper owner simplification
  - Type: seam-refactor + shared-runtime + formal-spec
  - Owner: field descriptors for render metadata; `Uploads/UploadValue` for pure upload value shape helpers; `Config` for strict array-path reads; `Logging/FileSink` for shared file-write mechanics; `SubmitHandler` for spam short-circuit orchestration.
  - Done When: `FormRenderer` consumes descriptor `enterkeyhint` metadata; normalizer, validator, store, and email upload paths route shared shape/name extraction through `UploadValue` while preserving email safe-name strictness; config readers use strict `Config::value()`/`Config::bool()` semantics; JSONL and fail2ban logging keep family naming in their concrete loggers while sharing locked append/prune mechanics; honeypot and spam-fail branches share cleanup flow without changing status, trace, ledger burn, logging, or stealth/hard-fail behavior.
  - Verified via: `php eforms/tests/unit/test_upload_value.php`; `php eforms/tests/unit/test_config_validation.php`; `php eforms/tests/unit/test_protocol_seam_guards.php`; `php eforms/tests/integration/test_email_upload_value_strictness.php`; `php eforms/tests/integration/test_email_attachments_policy.php`; `php eforms/tests/integration/test_spam_fail_threshold.php`; `php eforms/tests/integration/test_honeypot_paths.php`; `php eforms/tests/integration/test_logging_jsonl_schema.php`; `php eforms/tests/integration/test_fail2ban_line_format.php`; `find eforms/tests/unit eforms/tests/integration eforms/tests/smoke -type f -name 'test_*.php' -print0 | sort -z | xargs -0 -n1 php`; `php eforms/tests/wp-runtime/run.php`; `php eforms/tests/tools/assert-template-slugs.php`.
  - Reasoning: high

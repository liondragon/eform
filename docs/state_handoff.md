...
Only run residue that is not captured well by the active owner docs, code, or tests.

## Non-Obvious Implementation Notes
- Throttle ownership is centralized in `Security::enforce_throttle()` + `Throttle::check()`; mint/validate entrypoints consume that API and should not manipulate throttle files directly.
...
- Logging is now sink-routed in `Logging::event()`: minimal and JSONL follow `logging.mode`, while Fail2ban emission is independent of `logging.mode` and only writes for `EFORMS_ERR_*` codes.

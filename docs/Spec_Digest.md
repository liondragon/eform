...
- [ ] Email fail after ledger reserve → burn original in ledger, send admin notice, redirect to virtual failure page without retry token. → [§Security](../docs/Canonical_Spec.md#sec-security)
...
- [ ] Tokens reuse on local error rerender; email failure is a result-page redirect and mints no retry token. → [§Security](../docs/Canonical_Spec.md#sec-security)
...
- [ ] Success uses virtual result pages; no inline success cacheability gate or external result redirect. → [§Success](../docs/Canonical_Spec.md#sec-success)
...
- [ ] Reserved: form_id, instance_id, submission_id, eforms_token, eforms_hp, eforms_mode, timestamp, js_ok, ip, submitted_at. → [§Template](../docs/Canonical_Spec.md#sec-template-model-fields)

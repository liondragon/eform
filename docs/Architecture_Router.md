...
This repo uses this router, `docs/Owner_Index.md`, `README.md`, `docs/contracts/*`, affected owner docs/READMEs, code, and tests as the active implementation contracts.

## Project Doctrine

- Prefer one canonical owner for reusable contracts.
- Keep durable operator-facing behavior in `README.md`; keep owner lookup here and in `docs/Owner_Index.md`; keep executable detail in code and tests.
...
- Follow the canonical lifecycle and compatibility policy in `AGENTS.md`. While it declares GREENFIELD, replace superseded contracts cleanly; do not add migration or compatibility paths, duplicate config/schema owners, or local bypasses for a documented owner unless the user explicitly requires them.
...
- Managed uploads: `UploadBatchStore` owns authorized intents, declared-byte reservations, stored items, tombstones, immutable storage/deployment identity, staged/finalized aggregate state, capability-gated finalized-submission availability updates and deletion, remote purge traversal, and the external capacity facade. `ManagedArtifactKey` owns the canonical batch-scoped object namespace and declared-MIME extension; `worker/src/managed-artifact-key.js` is its Worker peer. Store-private `ManagedCapacityStore` owns capacity-record persistence plus storage-owned artifact and transient-allocation reservation arithmetic; `LocalArtifactStore` owns write-once local artifact bytes and durable deletion fences. `UploadPolicy` owns accepted-media policy and delegates box- and association-bounded, decoder-free HEIC/HEIF container inspection to `HeifInspector`; `worker/src/heif.js` enforces that same policy asynchronously for Worker artifacts. `WorkerProtocol` and `worker/src/protocol.js` own the environment-bound PHP/edge signing contract; `WorkerClient` owns bounded signed WordPress-to-Worker status, review, cleanup, inspect, and health operations and rejects an aggregate fingerprint that does not match the configured Worker origin/environment before network access. The Worker owns conditional R2 writes, awaited validation-Queue publication, immutable exact-object terminal results, bounded gallery status, accepted-only review delivery, and exact result/artifact deletion, but no aggregate state. `worker/src/validation-result.js` is the Worker-local key and strict result-codec owner shared by Queue, status, review, and cleanup handlers. `LocalPreviewProvider` owns optional local preview-cache production, producer locks, the bounded global semaphore, and bounded reclamation of safely aged preview-deletion fences. The facade retains aggregate traversal and object-budget-before-aggregate lock ordering. `UploadBatchEndpoint` owns batch HTTP requests; `ReviewController` owns signed gallery and local file/preview reads plus operator availability/deletion gates and short-lived Worker review grants.
...
- Browser assets: `assets/forms.js` owns client enhancement, JS minting, submit blocking, error focus behavior, the staged-photo same-document submission consumer, and the single phase-derived staged-upload scheduler across local multipart and Worker transports. Item phase derives shared body-transfer and provider-specific pipeline occupancy from `Anchors`-owned bounds; no mutable admission counter or transport-local queue is authoritative. Plugin CSS is split by runtime surface into core form, managed upload, review gallery, and admin settings bundles.
...
- WordPress signs Worker upload, gallery-status, review, object, and health capabilities and verifies Worker receipts/results through `WorkerProtocol`; the Worker never calls aggregate mutation methods, and browser-carried tokens never become authority outside their signed scope. Queue jobs and terminal results remain inside the deployment-owned Worker/R2/Queue boundary.
...
- Production artifact ingress, validation, review, and lifecycle: `worker/src/index.js` handles exact-origin preflight, signed write-once R2 upload, awaited Queue publication, Queue consumption, bounded signed gallery status, accepted-only private preview/download delivery, signed non-customer binding health, and exact inspect/delete operations. `worker/src/media.js` is called by the Queue consumer, never by ingress. WordPress and `forms.js` drive signed ingress and immediate stored-receipt registration; `ReviewController` asks for one finalized-manifest status snapshot before rendering and mints exact manifest-bound review grants, while the Worker independently enforces accepted terminal-result delivery; `GcRunner` and uninstall adapt `UploadBatchStore` drained cleanup through `WorkerClient`.
- Managed review: `ReviewController` owns the stateless versioned `/review/{token}` codec and validates opaque gallery/member bearers, renders accepted, processing, or unavailable cards, and admits capability/nonce-gated availability updates and whole-submission deletion through `UploadBatchStore`. Member links remain WordPress-owned while the finalized manifest is available; each Worker-owned member request mints one fresh short-lived exact-object action from manifest authority and redirects, while the Worker enforces accepted-result delivery; local members stream artifacts/previews directly. Worker status is bounded presentation evidence and is never persisted or accepted as media-access authority. `LocalPreviewProvider` and the Worker produce optional representations without manifest mutation. Persisted aggregate ownership, not the currently configured composition, selects review and cleanup storage.

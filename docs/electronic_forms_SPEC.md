...
6. CENTRAL REGISTRIES (INTERNAL ONLY)
	...
	-	<a id="sec-lazy-load-matrix"></a>Lazy-load lifecycle (components & triggers):
		...
		|------------------|------------:|-------------------------|-------|
		| Config snapshot | Lazy | First `Config::get()` call (entry points such as `FormRenderer::render()`, `SubmitHandler::handle()`, `Security::token_validate()`, `Emailer::send()`, **`/eforms/prime`**, and **`/eforms/success-verify`** invoke it before helpers) | Idempotent (per request); entry points decide when to create the snapshot; see [Configuration: Domains, Constraints, and Defaults (§17)](#sec-configuration) for bootstrap timing. |

 ...
 10. Throttling (optional; file-based)
  ...
  11. Adaptive challenge (optional; Turnstile preferred)
	...
	- Render only on POST re-render when required (or always); never on initial GET unless §7.1 requires challenge.
        - In cookie mode, challenge rerenders MUST clear the `eforms_eid_{form_id}` cookie (as described in §7.1.4) and then include the `/eforms/prime?f={form_id}[&s={slot}]` pixel so `/eforms/prime` observes the missing cookie and mints a fresh EID before the next POST per §7.1.1/§7.1.4 (the explicit exception to §7.1.4's no-rotation rule).

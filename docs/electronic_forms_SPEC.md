  ...
  1. Submission Protection for Public Forms (hidden vs cookie)
    ...
    - Canonical pipeline (render → persist → POST → rerender) shared by renderer, submit handler, and QA:
      ...
      3. **POST `/eforms/submit`**
        ...
        - Validation exposes `{ mode:\"hidden\"|\"cookie\", submission_id:\"…\", slot?:int, token_ok:bool, hard_fail:bool, soft_signal:0|1, require_challenge:bool }`. Hidden mode reports the raw token; cookie mode reports the EID (plus slot suffix). Renderer and downstream components (logging, throttling, success tickets) consume this structure. When `security.cookie_missing_policy="challenge"` handles a missing cookie, the validator sets `soft_signal=1` and `require_challenge=true` so §7.11's adaptive challenge handshake can run before delivery. For NCIDs, `submission_id` is the "nc-" value described above.
      4. **Rerender and rotation rules**
        - Cookie-mode rerenders normally reuse the minted `{eid, slot}` tuple until expiry or success. Challenge flows (either security.cookie_missing_policy="challenge" or a configured challenge mode that demands a rerender) MUST include `/eforms/prime?f={form_id}[&s={slot}]` on the rerender to mint a fresh EID before the next POST (explicit exception to §7.1.4’s no-rotation rule). Otherwise, no mid-flow rotation occurs.
 ...
 10. Throttling (optional; file-based)
  ...
  11. Adaptive challenge (optional; Turnstile preferred)
	...
	- Render only on POST re-render when required (or always); never on initial GET unless §7.1 requires challenge.
        - In cookie mode, challenge rerenders MUST include the `/eforms/prime?f={form_id}[&s={slot}]` pixel so a fresh EID is minted before the next POST (the explicit exception to §7.1.4's no-rotation rule).
...
13. SUCCESS BEHAVIOR (PRG)
  ...
  - **Success UX without cookies (NCID flow)**: When the submission proceeded under an NCID (no acceptable cookie), implementations MUST use `success.mode="redirect"` to a non-cached endpoint (or an endpoint that performs server-side verification) because the short-lived success cookie may be blocked by the client. Inline success on a cached page MUST NOT be used in this case.

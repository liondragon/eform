...
- Definition — Cookie reissue scope = `/eforms/prime` reissues an identifier only when an unexpired record exists and the request lacks an unexpired match; it MUST skip the positive header whenever an unexpired match is present.

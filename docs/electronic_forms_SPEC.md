  ...
  1. Submission Protection for Public Forms (hidden vs cookie)
    ...
    - Canonical pipeline (render → persist → POST → rerender) shared by renderer, submit handler, and QA:
      ...
      4. Rerender and rotation rules
        - Cookie-mode rerenders normally reuse the minted `{eid, slot}` tuple until expiry or success. Challenge flows (either security.cookie_missing_policy="challenge" or a configured challenge mode that demands a rerender) MUST include `/eforms/prime?f={form_id}&remint=1[&s={slot}]` on the rerender so `/eforms/prime` forces a fresh EID before the next POST (explicit exception to §7.1.4’s no-rotation rule). Otherwise, no mid-flow rotation occurs.
 ...
 10. Throttling (optional; file-based)
  ...
  11. Adaptive challenge (optional; Turnstile preferred)
	...
	- Render only on POST re-render when required (or always); never on initial GET unless §7.1 requires challenge.
        - In cookie mode, challenge rerenders MUST include the `/eforms/prime?f={form_id}&remint=1[&s={slot}]` pixel so a fresh EID is minted before the next POST per §7.1.1/§7.1.4 (the explicit exception to §7.1.4's no-rotation rule).

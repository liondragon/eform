...
7. SECURITY
  ...
  6. Test/QA Matrix (v4.4 mandatory)
    ...
    - Cookie-mode checks:
      ...
      - Drop the cookie and rely on `security.cookie_missing_policy="soft"` → continue submission flow and log the soft signal; `"hard"` must block (hard fail) per policy and `"challenge"` must emit the soft signal while forcing the §7.11 challenge handshake (hard fail only if the challenge cannot be satisfied).

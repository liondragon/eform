...
Email delivery uses `wp_mail()`. SMTP transport details (retries/backoff, DKIM signing, provider debug transcripts), and staging-redirect/disable guards are delegated to the site's mail plugin/MTA rather than implemented inside this plugin.

### Removed: logging.on_failure_canonical

Logging form field values on validation failure was deemed low-value and privacy-adjacent. Operators needing post-mortem debugging can temporarily enable `logging.pii=true` and reproduce the issue.

### Applied: Unconditional novalidate Attribute

Renderer always includes `novalidate` to prevent browser-native validation from conflicting with server-rendered errors. The `html5.client_validation` toggle was removed to simplify the contract.

...
$gallery_pattern = '#https://example\.test/review/[A-Za-z0-9_-]{44}#';
eforms_test_assert( preg_match_all( $gallery_pattern, $mail['message'] ) === 1 && strpos( $mail['message'], 'expires' . '=' ) === false, 'A staged field should contribute exactly one expiration-free opaque gallery URL to text email.' );
...
eforms_test_assert( strpos( $mail['message'], '/review/file/' ) === false && $mail['attachments'] === array(), 'Staged photos should produce neither individual file links nor attachments.' );
...
eforms_test_assert( substr_count( $failed_mail['message'], 'Review photos' ) === 1 && preg_match_all( $gallery_pattern, $failed_mail['alt_body'] ) === 1, 'HTML and text alternatives should each contain one gallery summary.' );
